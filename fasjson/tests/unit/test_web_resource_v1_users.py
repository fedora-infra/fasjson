...
def test_search_user_success_list(client, gss_user, mock_ldap_client):
    data = [_get_user_ldap_data(f"dummy-{idx}") for idx in range(1, 10)]
    result = LDAPResult(items=data)
    mock_ldap_client(search=lambda page_size, page_number, model, filters, sub_dn: result,)

    rv = client.get("/v1/users/search/dummy/")
...
def test_search_user_success_single(client, gss_user, mock_ldap_client):
    ...
    rv = client.get("/v1/users/search/dummy-1/")
...
def test_search_user_not_found(client, gss_user, mock_ldap_client):
    ...
    rv = client.get("/v1/users/search/somethingobscure/")

    assert 200 == rv.status_code
    assert rv.get_json() == {"result": []}


def test_user_search_error(client, gss_user, mock_ldap_client):
    mock_ldap_client(search=lambda page_size, page_number, model, filters, sub_dn: None,)

    rv = client.get("/v1/users/search/something/")
