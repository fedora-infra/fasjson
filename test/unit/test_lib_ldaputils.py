...
def test_whoami(mocker):
    ...
    L.return_value = types.SimpleNamespace(
        ...
        whoami_s=lambda: r)
    l = ldaputils.LDAP('https://dummy.com')
...
def test_get_groups(mocker):
    ...
    def result3(msgid, resp_ctrl_classes=''):
        return 101, mocked, 2, [SimplePagedResultsControl(True, size=20, cookie='')]
    ...
    assert l.get_groups() == (2, 0, None, ['admins', 'ipausers', 'editors', 'trust admins'])
...
def test_get_group_members(mocker):
    ...
    def search_ext(a, b, c, attrlist='', serverctrls=''):
        return mocked
    def result3(msgid, resp_ctrl_classes=''):
        return 101, mocked, 1, [SimplePagedResultsControl(True, size=20, cookie='')]
    ...
    assert l.get_group_members('admins') == (1, 0, None, ['admin'])
...
def test_get_user(mocker):
    ...
    assert  expected == l.get_user('admin')